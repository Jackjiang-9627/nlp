...
class BertAdam(Optimizer):
    def __init__(self, params, lr=1e-4, warmup=-1, t_total=-1, schedule='warmup_linear',
        ...
        """
        实现bert版本的带权重衰减的Adam算法
        :param params: 可迭代的tensor对象或字典对象 Specifies what Tensors should be optimized.
        :param lr: 学习率
        :param warmup: portion of t_total for the warmup, -1 means no warmup. Default: -1
        :param t_total: 训练总批次（一个批次调整一次学习率） 默认-1：lr=1
        :param schedule: 'warmup_linear'【默认】、'warmup_constant、'warmup_cosine'、`_LRSchedule`
                        'none'、`None`:学习率为常数
        :param b1: Adams b1. Default: 0.9
        :param b2: Adams b2. Default: 0.999
        :param e: Adams epsilon. Default: 1e-6
        :param weight_decay: 权重衰减 Default: 0.01
        :param max_grad_norm: 梯度截断 (-1 means no clipping) Default: 1.0
        """
        if lr < 0.0:
